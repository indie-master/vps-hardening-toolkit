...
if [[ -n "${VPS_HARDENING_NGINX_SH:-}" ]]; then
  return 0
fi
...
nginx::configure_official_repo() {
  local codename="$1"
  local tmp_key fingerprint
  tmp_key="$(mktemp)"
  trap 'rm -f "$tmp_key"' RETURN

  curl -fsSL https://nginx.org/keys/nginx_signing.key -o "$tmp_key"
  fingerprint="$(gpg --show-keys --with-colons "$tmp_key" 2>/dev/null | awk -F: '$1=="fpr" {print $10; exit}')"
  ...
  cat > "$NGINX_LIST" <<EOF
deb [signed-by=$NGINX_KEYRING] https://nginx.org/packages/ubuntu $codename nginx
EOF
  cat > "$NGINX_PIN" <<'EOF'
Package: *
...
nginx::configure_repo() {
  ...
  version="$(nginx::ubuntu_version)"
  codename="$(nginx::codename)"

  case "$version" in
    22.04|24.04)
      nginx::configure_official_repo "$codename"
      ;;
    20.04)
...
EOF
  ...
  log::ok "Base nginx configuration written."
}
...
nginx::status() {
  if ! command -v nginx >/dev/null 2>&1; then
    log::warn "nginx is not installed."
    return 0
  fi
